...
{block name="label"}
  {if $input.name == 'zipcode' && isset($input.label)}
    <label id="zipcode-label" class="control-label col-lg-3">{$input.label|escape:'htmlall':'UTF-8'}</label>
  {elseif $input.name == 'states[]'}
    <label id="states-label" class="control-label col-lg-3">{$input.label|escape:'htmlall':'UTF-8'}</label>
  {else}
    {$smarty.block.parent}
  {/if}
{/block}
